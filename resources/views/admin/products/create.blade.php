...
@section('header_title', 'Agregar Producto')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 sm:p-8">
        <h2 class="text-lg font-bold text-slate-800 mb-6">Detalles del Producto</h2>
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6 p-4 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50">
                    ...
                    <div class="flex-1 text-center sm:text-left space-y-2">
                        ...
                        <p class="text-xs text-slate-400">Tamaño máximo recomendado: 5MB.</p>
                    </div>
                </div>
                @error('image')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                    ...
                    placeholder="Ej. Laptop Lenovo IdeaPad 3, Mouse Logitech G502">
                @error('name')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" min="0" name="min_price" id="min_price" value="{{ old('min_price') }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition text-sm"
                    placeholder="Especificaciones clave, características, procesador, memoria, garantía...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
